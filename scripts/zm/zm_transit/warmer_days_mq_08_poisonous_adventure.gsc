...
spawn_drinkable_step()
{
    ...
    while( true )
    {
        ...
        if( initial_hit  )
        {
            ...
            level thread initial_zip_text_so_thread_doesnt_linger(); // this makes it so that the zip thread can re run while timing loop is taking a place.
            wait 1;
...
initial_zip_text_so_thread_doesnt_linger()
{
    level endon( "end_game" );
    wait 3;
    foreach( g in level.players ) { for( i = 0; i < 4; i++ ) { g playSound( level.jsn_snd_lst[ 20 ] );} }
